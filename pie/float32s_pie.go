...
func (ss Float32s) Abs() Float32s {
	result := make(Float32s, len(ss))
	for i, val := range ss {
		if val < 0 {
			result[i] = -val
		} else {
...
func (ss Float32s) All(fn func(value float32) bool) bool {
...
func (ss Float32s) Any(fn func(value float32) bool) bool {
	for _, value := range ss {
		if fn(value) {
			return true
		}
...
func (ss Float32s) Append(elements ...float32) Float32s {
	// Copy ss, to make sure no memory is overlapping between input and
	// output. See issue #97.
	result := append(Float32s{}, ss...)
...
// for sort.Float32sAreSorted.
func (ss Float32s) AreSorted() bool {
...
func (ss Float32s) Bottom(n int) (top Float32s) {
...
func (ss Float32s) Contains(lookingFor float32) bool {
	for _, s := range ss {
		if lookingFor == s {
			return true
		}
...
func (ss Float32s) Diff(against Float32s) (added, removed Float32s) {
	...
	diffOneWay := func(ss1, ss2raw Float32s) (result Float32s) {
		ss2 := make(Float32s, len(ss2raw))
		copy(ss2, ss2raw)
...
// if n < 0 it'll return empty slice.
func (ss Float32s) DropTop(n int) (drop Float32s) {
	if n < 0 || n >= len(ss) {
		return
	}
...
func (ss Float32s) DropWhile(f func(s float32) bool) (ss2 Float32s) {
	ss2 = make([]float32, len(ss))
	copy(ss2, ss)
...
//   })
//
func (ss Float32s) Each(fn func(float32)) Float32s {
	for _, s := range ss {
		fn(s)
	}
...
func (ss Float32s) Equals(rhs Float32s) bool {
	if len(ss) != len(rhs) {
		return false
	}
...
//
// It is acceptable to provide zero arguments.
func (ss Float32s) Extend(slices ...Float32s) (ss2 Float32s) {
	ss2 = ss
...
func (ss Float32s) Filter(condition func(float32) bool) (ss2 Float32s) {
	for _, s := range ss {
		if condition(s) {
			ss2 = append(ss2, s)
		}
...
func (ss Float32s) FilterNot(condition func(float32) bool) (ss2 Float32s) {
	for _, s := range ss {
		if !condition(s) {
			ss2 = append(ss2, s)
		}
...
func (ss Float32s) FindFirstUsing(fn func(value float32) bool) int {
...
func (ss Float32s) First() float32 {
	return ss.FirstOr(0)
}

// FirstOr returns the first element or a default value if there are no
// elements.
func (ss Float32s) FirstOr(defaultValue float32) float32 {
	if len(ss) == 0 {
		return defaultValue
	}
...
func (ss Float32s) Float64s() Float64s {
	l := len(ss)
...
//
func (ss Float32s) Group() map[float32]int {
	group := map[float32]int{}
	for _, n := range ss {
		group[n]++
	}
	return group
...
func (ss Float32s) Intersect(slices ...Float32s) (ss2 Float32s) {
	if slices == nil {
		return nil
	}

	var uniqs = make([]map[float32]struct{}, len(slices))
...
func (ss Float32s) Insert(index int, values ...float32) Float32s {
	if index >= ss.Len() {
		return Float32s.Extend(ss, Float32s(values))
	}

	return Float32s.Extend(ss[:index], Float32s(values), ss[index:])
}

// Ints transforms each element to an integer.
func (ss Float32s) Ints() Ints {
	l := len(ss)
...
func (ss Float32s) Join(glue string) (s string) {
	var slice interface{} = []float32(ss)
...
func (ss Float32s) JSONBytes() []byte {
	if ss == nil {
		return []byte("[]")
	}
...
func (ss Float32s) JSONBytesIndent(prefix, indent string) []byte {
	if ss == nil {
		return []byte("[]")
	}
...
// One important thing to note is that it will treat a nil slice as an empty
// slice to ensure that the JSON value return is always an array.
func (ss Float32s) JSONString() string {
	if ss == nil {
		return "[]"
	}
...
// slice to ensure that the JSON value return is always an array. See
// json.MarshalIndent for details.
func (ss Float32s) JSONStringIndent(prefix, indent string) string {
	if ss == nil {
		return "[]"
	}
...
func (ss Float32s) Last() float32 {
	return ss.LastOr(0)
}

// LastOr returns the last element or a default value if there are no elements.
func (ss Float32s) LastOr(defaultValue float32) float32 {
	if len(ss) == 0 {
		return defaultValue
	}
...
func (ss Float32s) Map(fn func(float32) float32) (ss2 Float32s) {
	if ss == nil {
		return nil
	}
...
func (ss Float32s) Max() (max float32) {
	if len(ss) == 0 {
		return
	}
...
//
// If the number of elements is even, then the float32 mean of the two "median values"
// is returned.
func (ss Float32s) Median() float32 {
	n := len(ss)
	if n == 0 {
		return 0
	}
...
func (ss Float32s) Min() (min float32) {
	if len(ss) == 0 {
		return
	}
...
func (ss Float32s) Mode() Float32s {
	if len(ss) == 0 {
		return nil
	}
	values := make(map[float32]int)
	for _, s := range ss {
		values[s]++
	}
...
//   }
func (ss *Float32s) Pop() (popped *float32) {
...
func (ss Float32s) Product() (product float32) {
	if len(ss) == 0 {
		return
	}
	product = ss[0]
...
func (ss Float32s) Random(source rand.Source) float32 {
	n := len(ss)
...
//
// Returns a zero value of float32 if there are no elements in the slice. It will panic if the reducer is nil and the slice has more than one element (required to invoke reduce).
// Otherwise returns result of applying reducer from left to right.
func (ss Float32s) Reduce(reducer func(float32, float32) float32) (el float32) {
	if len(ss) == 0 {
		return
	}
	el = ss[0]
...
//   ss.Sort().Reverse()
//
func (ss Float32s) Reverse() Float32s {
...
func (ss Float32s) Send(ctx context.Context, ch chan<- float32) Float32s {
	for i, s := range ss {
		select {
		case <-ctx.Done():
			return ss[:i]
...
func (ss Float32s) Sequence(params ...int) Float32s {
	var creator = func(i int) float32 {
		return float32(i)
	}
...
// where min is the first param, max is the second, step is the third one, [min, max) with step,
// others params will be ignored
func (ss Float32s) SequenceUsing(creator func(int) float32, params ...int) Float32s {
	var seq = func(min, max, step int) (seq Float32s) {
		lenght := int(util.Round(float64(max-min) / float64(step)))
		if lenght < 1 {
			return
		}

		seq = make(Float32s, lenght)
...
func (ss Float32s) Shift() (float32, Float32s) {
	return ss.First(), ss.DropTop(1)
}

// Shuffle returns shuffled slice by your rand.Source
func (ss Float32s) Shuffle(source rand.Source) Float32s {
	n := len(ss)
...
// Sort works similar to sort.Float32s(). However, unlike sort.Float32s the
// slice returned will be reallocated as to not modify the input slice.
//
// See Reverse() and AreSorted().
func (ss Float32s) Sort() Float32s {
	// Avoid the allocation. If there is one element or less it is already
	// sorted.
	if len(ss) < 2 {
		return ss
...
func (ss Float32s) Stddev() float64 {
	if len(ss) == 0 {
		return 0.0
	}
...
//   fmt.Sprintf("%v")
//
func (ss Float32s) Strings() Strings {
	l := len(ss)
...
func (ss Float32s) SubSlice(start int, end int) (subSlice Float32s) {
	if start < 0 || end < 0 {
		return
	}
	...
	if start < length {
		if end <= length {
			...
			zeroArray := make([]float32, end-length)
			subSlice = ss[start:length].Append(zeroArray[:]...)
		}
	} else {
		zeroArray := make([]float32, end-start)
		subSlice = zeroArray[:]
	}
...
func (ss Float32s) Sum() (sum float32) {
	for _, s := range ss {
		sum += s
	}
...
// if the slice has less elements then n that'll return all elements
// if n < 0 it'll return empty slice.
func (ss Float32s) Top(n int) (top Float32s) {
...
func (ss Float32s) StringsUsing(transform func(float32) string) Strings {
	l := len(ss)
...
//
// See AreUnique().
func (ss Float32s) Unique() Float32s {
	// Avoid the allocation. If there is one element or less it is already
	// unique.
	if len(ss) < 2 {
		return ss
	...
	values := map[float32]struct{}{}

	for _, value := range ss {
		values[value] = struct{}{}
	}

	var uniqueValues Float32s
...
func (ss Float32s) Unshift(elements ...float32) (unshift Float32s) {
	unshift = append(Float32s{}, elements...)
