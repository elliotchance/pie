...
type CarsConditionFunc func(Car) bool

type CarsTransformFunc func(Car) Car

// The functions below work for all slices types.
...
func (ss Cars) Only(condition CarsConditionFunc) (ss2 Cars) {
	for _, s := range ss {
		if condition(s) {
			ss2 = append(ss2, s)
		}
...
func (ss Cars) Without(condition CarsConditionFunc) (ss2 Cars) {
	for _, s := range ss {
		if !condition(s) {
			ss2 = append(ss2, s)
		}
...
func (ss Cars) Transform(fn CarsTransformFunc) (ss2 Cars) {
	if ss == nil {
		return nil
	}
